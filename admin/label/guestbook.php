...
if($action=='mod'){
	...
	$postdb[tplpart_1code]=str_replace('$url','$webdb[www_url]/do/guestbook.php?fid=$fid&id=$id#$id',$postdb[tplpart_1code]);
	$postdb[tplpart_2code]=str_replace('$url','$webdb[www_url]/do/guestbook.php?fid=$fid&id=$id#$id',$postdb[tplpart_2code]);
	...
	$SQL=" SELECT A.*,content AS title FROM {$pre}guestbook A $SQL ORDER BY A.id DESC LIMIT $sqlmin,$rows ";
	...
	require("head.php");
	require("template/label/guestbook.htm");
	require("foot.php");
