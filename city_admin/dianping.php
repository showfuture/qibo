...
if($action=='del'){
	$rsdb=$db->get_one("SELECT C.*,A.city_id FROM {$_pre}db A LEFT JOIN {$_pre}dianping C ON A.id=C.id WHERE A.city_id='$city_id' AND C.cid='$cid'");
	if(!$rsdb){
		showerr('ID有误!');
	}
...
$query = $db->query("SELECT SQL_CALC_FOUND_ROWS C.*,A.city_id FROM {$_pre}db A LEFT JOIN {$_pre}dianping C ON A.id=C.id WHERE A.city_id='$city_id' AND C.cid>0 ORDER BY C.cid DESC LIMIT $min,$rows");
...
require(dirname(__FILE__)."/template/dianping.htm");
